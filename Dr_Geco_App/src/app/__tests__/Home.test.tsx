...
describe('Home',()=>{
    test('test render content',()=>{
        ...
        expect(screen.getByText('Great Writing,Simplified')).toBeInTheDocument()
        expect(screen.getByText("Compose bold, clear, mistake-free writing with Dr. Geco’s new AI-powered desktop Windows app.")).toBeInTheDocument()
        expect(screen.getByTestId("policy")).toBeInTheDocument()
    })
    test('test links', ()=>{
        render(<Home />)
        const registerLink=screen.getAllByRole('link',{name:"Get Dr. Geco it's free"})
